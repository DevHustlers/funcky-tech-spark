...
function hslVarToHex(hslString: string): string {
...
const LightRays = ({
  ...
  // Derive ray color from the CSS variable --foreground
  const getRaysColor = useCallback(() => {
    try {
  ...
  useEffect(() => {
    ...
    isVisible, raysOrigin, getRaysColor, raysSpeed, lightSpread, rayLength,
    pulsating, fadeDistance, saturation, followMouse, mouseInfluence, noiseAmount, distortion
  ]);

  // Update uniforms when props/theme change without full re-init
  useEffect(() => {
    if (!uniformsRef.current || !containerRef.current || !rendererRef.current) return;
    const u = uniformsRef.current;
    const renderer = rendererRef.current;
    u.raysColor.value = hexToRgb(getRaysColor());
    ...
    getRaysColor, raysSpeed, lightSpread, raysOrigin, rayLength,
